@extends('master')
@section('title','Edit - '.$editWardUser->name)
@section('content')
<div class="maininnersection">
    <form class="text-center" id="parshadCreate" action="{{route('admin.create.ward.user.process')}}" method="post">
        @csrf
        <p class="titlesection">Edit Ward User</p>

        <div class="form-row mb-2" data-validate="Name is required">
            <label for="name" class="labelinput">Name</label>
            <input type="text" id="name" name="name" value="{{$editWardUser->name}}"
        ...
        <div class="form-row mb-2" data-validate="Mobile no is not valid">
            ...
            <input type="text" id="mobile" name="mobile" value="{{$editWardUser->mobile}}"
        ...
        <div class="form-row mb-2" data-validate="Email is not valid">
            ...
            <input type="text" id="email" name="email" value="{{$editWardUser->email}}"
                class="input_section form-control mb-0 validate_this" />
        </div>
        <div class="form-row mb-2" data-validate="Password is required">
            <label for="password" class="labelinput">Password</label>
            <input type="text" id="password" name="password" value="{{$editWardUser->password2}}"
                class="input_section form-control mb-0 validate_this " />
        </div>
        <div class="form-row mb-2" data-validate="Select a state">
            ...
            <select class="browser-default custom-select input_section selet validate_this"
                ...
                <option value="{{$item->id}}" @if ($item->id == $editWardUser->state)
                    selected
        ...
        <div class="row mx-0">
            <div class="width-50">
                <div class="form-row mb-4" data-validate="Ward no required">
                    ...
                    <input type="text" id="ward_no" name="ward_no" value="{{$editWardUser->ward_no}}"
            ...
            <div class="width-50">
                <div class="form-row mb-4" data-validate="Ward name required">
                    ...
                    <input type="text" id="ward_name" name="ward_name" value="{{$editWardUser->ward_name}}"
                        class="input_section validate_this form-control mb-0" />
                </div>
            </div>
        </div>
        <input type="hidden" name="id" value="{{$editWardUser->id}}">
        <div class="submitbtn">
            <button class="submitbutton btn btn-dark btn-block submit_button" data-id="parshadCreate" type="submit">
                Update
            </button>
            <a class="submitbutton btn btn-danger btn-block" href="{{route('admin.list.warduser')}}">
                Cancel
            </a>
        </div>
...
<script>
    $(document).ready(function(){
        getCity($('#state'),'{{route('getCity')}}','city','{{$editWardUser->city}}');
    });
